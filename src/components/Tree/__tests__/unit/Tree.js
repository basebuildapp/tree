...
describe('@atlaskit/tree - Tree', function () {
    ...
    describe('#render', function () {
        ...
        it('renders Draggable around each item if drag and drop is enabled', function () {
            var tree = enzyme_1.mount(<Tree_1["default"] tree={treeWithThreeLeaves_1.treeWithThreeLeaves} renderItem={mockRender} isDragEnabled={true}/>);
            var draggables = tree.find(react_beautiful_dnd_next_1.Draggable);
            expect(draggables).toHaveLength(3);
        });
        it('renders Draggable around each item if drag and drop is conditionally enabled using a function', function () {
            var isDragEnabled = function () { return false; };
            var tree = enzyme_1.mount(<Tree_1["default"] tree={treeWithThreeLeaves_1.treeWithThreeLeaves} renderItem={mockRender} isDragEnabled={isDragEnabled}/>);
            var draggables = tree.find(react_beautiful_dnd_next_1.Draggable);
            expect(draggables).toHaveLength(3);
        });
        it('does not render Draggable around each item if drag and drop is explicitly disabled', function () {
            var tree = enzyme_1.mount(<Tree_1["default"] tree={treeWithThreeLeaves_1.treeWithThreeLeaves} renderItem={mockRender} isDragEnabled={false}/>);
            var draggables = tree.find(react_beautiful_dnd_next_1.Draggable);
